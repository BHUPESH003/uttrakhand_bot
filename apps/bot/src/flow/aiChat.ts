/**
 * Handles one turn of the AI_CHAT state (see ai-handoff-contract.html) —
 * called by engine.ts instead of a normal state's handleInput, because
 * rendering the reply means sending whatever the AI service just returned
 * (a dynamic, per-turn message list), not a fixed shape a FlowState's
 * onEnter can describe up front.
 *
...
async function sendFallback(
  ...
  // already routes to MAIN_MENU globally.
  await executeAction(client, to, {
    kind: "sendReplyButtons",
    body: t("ai_chat_error_body"),
...
export async function handleAiChatTurn(
  ...
  const request: AiConverseRequest = {
    ...
    message: {
      ...
      replyId: message.type === "text" ? null : (message.replyId ?? null),
      replyTitle: message.type === "text" ? null : (message.replyTitle ?? null),
    },
    context: {
